...
nation_bad_key as (
	...
	from nation
	left outer hash join region_good_join on (nation.n_regionkey = region_good_join.r_regionkey)
...
supplier_bad_key as (
	...
	from supplier
	inner hash join Candidates C on (supplier.s_suppkey = C.s_suppkey)
	...
	left outer hash join partsupp_good_join on 
...
supplier_good_join as (
	select 
		...
		C.p_partkey
	from supplier
	inner hash join Candidates C on (
