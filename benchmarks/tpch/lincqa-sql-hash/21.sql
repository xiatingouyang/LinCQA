...
lineitem_bad_key as (
	...
	from lineitem
	inner hash join Candidates C on (lineitem.l_orderkey = C.l_orderkey and lineitem.l_linenumber = C.l_linenumber)
	left outer hash join orders_good_join on (lineitem.l_orderkey = orders_good_join.o_orderkey)
	left outer hash join supplier_good_join on (lineitem.l_suppkey = supplier_good_join.s_suppkey and C.s_name = supplier_good_join.s_name)
